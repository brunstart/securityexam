...
class CustomUserDetailsServiceTest {
    ...
    @Test
    @DisplayName("존재하지 않는 사용자 아이디로 메소드를 실행했을 때 UsernameNotFoundException 발생")
    void loadUserByUsername_notFound(){
        // given
        UserRepository userRepository = mock(UserRepository.class); // 가짜 객체 생성
        CustomUserDetailsService customUserDetailsService
                = new CustomUserDetailsService(userRepository);

        when(userRepository.findByUsername("noname")).thenReturn(Optional.empty()); // findByUsername 메소드 사용시 thenReturn을 반환할것
    ...
    @Test
    @DisplayName("존재하는 username으로 메소드를 실행했을 때 CustomUserDetails로 잘 반환되는지 확인")
