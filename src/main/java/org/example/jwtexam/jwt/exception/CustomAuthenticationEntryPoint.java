...
public class CustomAuthenticationEntryPoint implements AuthenticationEntryPoint {
    ...
    public void commence(HttpServletRequest request, HttpServletResponse response, AuthenticationException authException) throws IOException, ServletException {
        ...
        // 1. request.getAttribute("exception")이 null일 경우 어떻게 처리할지
        // spring security가 전달한 AuthenticationException을 로그로 남긴다.
        if (exception == null) {
            log.error("Commence Occurred :: " + authException.getMessage());
        }

        // 요청이 뷰를 요청하는 것인지 데이터를 요청하는 것인지에 따라서 다르게 처리할 필요가 있다.
        // Restful로 요청한건지 그냥 페이지를 페이지로 요청할 것인지 구분
        if(isRestRequest(request)){
            // rest로 요청이 들어왔을 때 처리
            handleRestResponse(exception, request, response);
        }else{
            // 페이지로 요청이 들어왔을 때 처리
            handleResponse(exception, request, response, authException);
        }
    }
    ...
    private void handleRestResponse(String exception, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        // jwt를 이용해서 인증할 때 발생할 수 있는 예외들이 있을 것 같다
    ...
    // 페이지로 요청이 들어왔을 때 인증되지 않은 사용자라면 /loginform 으로 리다이렉션 하게 구현
    private void handleResponse(String exception, HttpServletRequest request, HttpServletResponse response, AuthenticationException authException) throws IOException, ServletException {
        log.error("Page Request - Commence : {}", exception);

        if (exception == null){
            // 추가적으로 할 일이 있다면 이곳에 구현 (예외처리 로직 등)
            log.error("Page Request - Commence : {}", exception);
        }else {
